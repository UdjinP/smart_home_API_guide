...
#### Controls
Контролируемые ноды.
Если требуется без сценария контролировать группу нодов одного типа, например несколько ламп, необходимо предварительно создать новую группу в таблице Node_groups, с указанием кода родительской группы (если 0 - корень). После этого добавить нужным нодам код созданной группы.
- id_ctrl
- id_node (код устройства)
- id_group (код группы нодов если их несколько / код ноды, если она одна)
